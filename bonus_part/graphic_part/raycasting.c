...
/*   Updated: 2022/12/26 13:40:29 by yamzil           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	writing_pxl_to_img3(t_map *list, int x, int y, t_data *data, int t)
{
	...
	color = data->da[t].addr[(int)(data->da[t].yofset * data->da[t].width + data->da[t].xofset)];
    if (color != 16711935)
...
void	texture1(t_data *data, t_cast *info, double y1, int t)
{
	...
	xofset = 0.0;
    if (info->hdoor)
		xofset = ((info->my_door->x - ((int)(info->my_door->x / TILE_SIZE) * TILE_SIZE)) * data->da[t].width) / TILE_SIZE;
	else if (info->vdoor)
		xofset = ((info->my_door->y - ((int)(info->my_door->y / TILE_SIZE) * TILE_SIZE)) * data->da[t].width) / TILE_SIZE;
	distance_top = y + (info->doorHeight / 2) - ((double)WIN_HEIGHT / 2);
	yofset = distance_top * data->da[t].height / info->doorHeight;
	data->da[t].xofset = abs((int)xofset);
	data->da[t].yofset = abs((int)yofset);
}
